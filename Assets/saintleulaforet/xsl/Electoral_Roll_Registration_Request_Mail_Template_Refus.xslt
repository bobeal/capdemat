				...
				<fo:flow flow-name="xsl-region-body">
					...
					<fo:block-container position="absolute" top="12cm" left="3.12cm" height="15.0cm" width="15.0cm">
						...
						<fo:block space-after.optimum="12.0pt" text-align="justify" font-size="12.0pt" font-family="serif">
							Je fais suite à votre demande d'inscription sur les listes électorales 
							du
							<xsl:for-each select="//cert:date_demande">
								<xsl:apply-templates />
							</xsl:for-each>
							pour vous informer qu'elle a été refusée :
						</fo:block>
						...
						<fo:block space-after.optimum="12.0pt" text-align="justify" font-size="12.0pt" font-family="serif">
							Vous avez la possibilité de me retourner les informations par courriel à l’adresse
							<fo:basic-link color="blue">
								<xsl:attribute name="external-destination">url("mailto:etatcivil@saint-leu-la-foret.fr?subject=<xsl:for-each select="//cert:type_demande">
										<xsl:apply-templates />
									</xsl:for-each> - <xsl:for-each select="//cert:numero_demande">
										<xsl:apply-templates />
									</xsl:for-each>
								")</xsl:attribute>
								etatcivil@saint-leu-la-foret.fr
							</fo:basic-link>
							 ou par courrier au service état civil de la mairie. Dans tous 
							les cas, veuillez rappeler le numéro de votre demande.
						</fo:block>
