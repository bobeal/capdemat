					...
					<fo:block-container position="absolute" top="12cm" left="3.12cm" height="15.0cm" width="15.0cm">
						...
						<fo:block space-after.optimum="12.0pt" text-align="justify" font-size="12.0pt" font-family="serif">
							...
							<fo:basic-link color="blue">
								<xsl:attribute name="external-destination">url("mailto:dsp@saint-leu-la-foret.fr?subject=<xsl:for-each select="//cert:type_demande">
										<xsl:apply-templates />
									</xsl:for-each> - <xsl:for-each select="//cert:numero_demande">
										<xsl:apply-templates />
									</xsl:for-each>
								")</xsl:attribute>
								dsp@saint-leu-la-foret.fr
							</fo:basic-link>
							 ou par courrier à la Direction des services à la population. Dans tous 
