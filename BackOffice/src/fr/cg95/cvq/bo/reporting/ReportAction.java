...
import fr.cg95.cvq.bo.action.BaseAction;
...
public class ReportAction extends BaseAction {
	...
	protected ActionForward executeLogic(
        ...
        if (reportType == null)
            reportType = wizardState.getTabId();

		ArrayList results = SearchManager.searchReportRecords(stateManager.getRequestManager(), filterForm, reportType);
		String currentSearch = (filterForm.getSearch() == null) ? "" : filterForm.getSearch();
        
		if ((results != null) || !currentSearch.equals("reporting")) {
            filterForm.setSearch(reportType);
            filterForm.setWholeResultsList(results);
        }
        // Set the current search in progres
		if (reportType.equals("report") || reportType.equals("reporting"))
		    stateManager.setReportSearch(filterForm);
        else if (!reportType.equals("edit") )
            stateManager.setReportFilter(filterForm);
        
        stateManager.setSelectedRecord(null);

        // We are looking for reports, so we display the report manager
		request.setAttribute(ManagerWizardState.NAME_REQUEST_PARAMETER, "report");
