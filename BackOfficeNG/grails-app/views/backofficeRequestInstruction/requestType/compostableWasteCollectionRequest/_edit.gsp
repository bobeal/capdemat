        ...
        <div class="yui-g">
          ...
          <div class="yui-u first">
           ...
           <g:render template="/backofficeRequestInstruction/widget/localReferentialDataStatic" 
              ...
              <dl>
                <dt class=""><g:message code="cwcr.property.collectionAddress.label" />  : </dt><dd id="collectionAddress" class="action-editField validate-address i18n-cwcr.property.collectionAddress" ><div><p class="additionalDeliveryInformation">${rqt?.collectionAddress?.additionalDeliveryInformation}</p><p class="additionalGeographicalInformation">${rqt?.collectionAddress?.additionalGeographicalInformation}</p><span class="streetNumber">${rqt?.collectionAddress?.streetNumber}</span> <span class="streetName">${rqt?.collectionAddress?.streetName}</span><g:if test="${!!rqt?.collectionAddress?.streetMatriculation}"><br /><em><g:message code="address.property.streetMatriculation" /></em><span class="streetMatriculation">${rqt?.collectionAddress?.streetMatriculation}</span></g:if><p class="placeNameOrService">${rqt?.collectionAddress?.placeNameOrService}</p><span class="postalCode">${rqt?.collectionAddress?.postalCode}</span> <span class="city">${rqt?.collectionAddress?.city}</span><p class="countryName">${rqt?.collectionAddress?.countryName}</p><g:if test="${!!rqt?.collectionAddress?.cityInseeCode}"><em><g:message code="address.property.cityInseeCode" /></em><span class="cityInseeCode">${rqt?.collectionAddress?.cityInseeCode}</span></g:if></div></dd>
              </dl>
