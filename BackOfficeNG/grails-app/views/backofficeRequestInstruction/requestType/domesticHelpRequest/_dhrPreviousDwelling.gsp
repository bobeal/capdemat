  ...
  <dl class="required">
      ...
      <dd id="dhrPreviousDwelling[${listSize - 1 - index}].dhrPreviousDwellingAddress" class="action-editField validate-address required-true i18n-dhr.property.dhrPreviousDwellingAddress" >
        <div><p class="additionalDeliveryInformation">${it?.dhrPreviousDwellingAddress?.additionalDeliveryInformation}</p><p class="additionalGeographicalInformation">${it?.dhrPreviousDwellingAddress?.additionalGeographicalInformation}</p><span class="streetNumber">${it?.dhrPreviousDwellingAddress?.streetNumber}</span> <span class="streetName">${it?.dhrPreviousDwellingAddress?.streetName}</span><g:if test="${!!it?.dhrPreviousDwellingAddress?.streetMatriculation}"><p class="streetMatriculation">Matricule: ${it?.dhrPreviousDwellingAddress?.streetMatriculation}</p></g:if><p class="placeNameOrService">${it?.dhrPreviousDwellingAddress?.placeNameOrService}</p><span class="postalCode">${it?.dhrPreviousDwellingAddress?.postalCode}</span> <span class="city">${it?.dhrPreviousDwellingAddress?.city}</span><p class="countryName">${it?.dhrPreviousDwellingAddress?.countryName}</p><g:if test="${!!it?.dhrPreviousDwellingAddress?.cityInseeCode}"><p class="cityInseeCode">INSEE: ${it?.dhrPreviousDwellingAddress?.cityInseeCode}</p></g:if></div>
      </dd>
