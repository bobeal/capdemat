  ...
  <dl class="required">
      ...
      <dd id="dhrRealAsset[${listSize - 1 - index}].dhrRealAssetAddress" class="action-editField validate-address required-true i18n-dhr.property.dhrRealAssetAddress" >
        <div><p class="additionalDeliveryInformation">${it?.dhrRealAssetAddress?.additionalDeliveryInformation}</p><p class="additionalGeographicalInformation">${it?.dhrRealAssetAddress?.additionalGeographicalInformation}</p><span class="streetNumber">${it?.dhrRealAssetAddress?.streetNumber}</span> <span class="streetName">${it?.dhrRealAssetAddress?.streetName}</span><g:if test="${!!it?.dhrRealAssetAddress?.streetMatriculation}"><br /><em><g:message code="address.property.streetMatriculation" /></em><span class="streetMatriculation">${it?.dhrRealAssetAddress?.streetMatriculation}</span></g:if><g:if test="${!!it?.dhrRealAssetAddress?.streetRivoliCode}"><br /><em><g:message code="address.property.streetRivoliCode" /></em><span class="streetRivoliCode">${it?.dhrRealAssetAddress?.streetRivoliCode}</span></g:if><p class="placeNameOrService">${it?.dhrRealAssetAddress?.placeNameOrService}</p><span class="postalCode">${it?.dhrRealAssetAddress?.postalCode}</span> <span class="city">${it?.dhrRealAssetAddress?.city}</span><p class="countryName">${it?.dhrRealAssetAddress?.countryName}</p><g:if test="${!!it?.dhrRealAssetAddress?.cityInseeCode}"><em><g:message code="address.property.cityInseeCode" /></em><span class="cityInseeCode">${it?.dhrRealAssetAddress?.cityInseeCode}</span></g:if></div>
      </dd>
