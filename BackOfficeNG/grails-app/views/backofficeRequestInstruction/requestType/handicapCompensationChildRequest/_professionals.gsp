  ...
  <dl class="condition-isProfessionals-filled">
      ...
      <dd id="professionals[${listSize - 1 - index}].professionalAddress" class="action-editField validate-address required-true i18n-hccr.property.professionalAddress" >
        <div><p class="additionalDeliveryInformation">${it?.professionalAddress?.additionalDeliveryInformation}</p><p class="additionalGeographicalInformation">${it?.professionalAddress?.additionalGeographicalInformation}</p><span class="streetNumber">${it?.professionalAddress?.streetNumber}</span> <span class="streetName">${it?.professionalAddress?.streetName}</span><g:if test="${!!it?.professionalAddress?.streetMatriculation}"><br /><em><g:message code="address.property.streetMatriculation" /></em><span class="streetMatriculation">${it?.professionalAddress?.streetMatriculation}</span></g:if><p class="placeNameOrService">${it?.professionalAddress?.placeNameOrService}</p><span class="postalCode">${it?.professionalAddress?.postalCode}</span> <span class="city">${it?.professionalAddress?.city}</span><p class="countryName">${it?.professionalAddress?.countryName}</p><g:if test="${!!it?.professionalAddress?.cityInseeCode}"><em><g:message code="address.property.cityInseeCode" /></em><span class="cityInseeCode">${it?.professionalAddress?.cityInseeCode}</span></g:if></div>
      </dd>
