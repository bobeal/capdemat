      ...
      <div id="page3">
        ...
        <div class="yui-g">
          ...
          <div class="yui-u first">
              ...
              <dl>
                <dt class="required"><g:message code="hsr.property.otherContactAddress.label" /> * : </dt><dd id="otherContactAddress" class="action-editField validate-address required-true i18n-hsr.property.otherContactAddress" ><div><p class="additionalDeliveryInformation">${rqt?.otherContactAddress?.additionalDeliveryInformation}</p><p class="additionalGeographicalInformation">${rqt?.otherContactAddress?.additionalGeographicalInformation}</p><span class="streetNumber">${rqt?.otherContactAddress?.streetNumber}</span> <span class="streetName">${rqt?.otherContactAddress?.streetName}</span><g:if test="${!!rqt?.otherContactAddress?.streetMatriculation}"><p class="streetMatriculation">Matricule: ${rqt?.otherContactAddress?.streetMatriculation}</p></g:if><p class="placeNameOrService">${rqt?.otherContactAddress?.placeNameOrService}</p><span class="postalCode">${rqt?.otherContactAddress?.postalCode}</span> <span class="city">${rqt?.otherContactAddress?.city}</span><p class="countryName">${rqt?.otherContactAddress?.countryName}</p><g:if test="${!!rqt?.otherContactAddress?.cityInseeCode}"><p class="cityInseeCode">INSEE: ${rqt?.otherContactAddress?.cityInseeCode}</p></g:if></div></dd>
              </dl>
