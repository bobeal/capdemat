...
<html>
  <head>
     <title>${message(code:'ipsr.description')}</title>
  ...
  <body>
    ...
    <div id="request" class="main-box">
      <h2>
        ...
        <g:message code="ipsr.label" /> <g:message code="message.number" args="[rqt.id]" />
      </h2>
      <p><g:message code="request.duration.label" /><strong> : <g:message code="ipsr.duration.value" /></strong></p>
      <p>
      ...
      <div class="datas">
         <g:set var="requestTypeAcronym" value="ipsr" scope="request" />
      ...
      <ul>
        ...
        <li class="${currentStep == 'iconito' ? 'current ' : ''}
          
            ${individual ? rqt.stepStates['iconito-' + params.type].state : rqt.stepStates['iconito'].state}
          
          ">
          <span class="number">1</span>
          <a
            <g:if test="${currentStep != 'iconito' && rqt.stepStates['iconito'].state != 'unavailable'}">
              href="${createLink(controller:'frontofficeRequest', action : 'edit', params:['id':rqt.id,'currentStep':'iconito'])}"
            </g:if>
          >
            
              <g:message code="${individual ? 'homeFolder.action.add' + org.apache.commons.lang.StringUtils.capitalize(params.type) : 'ipsr.step.iconito.label'}" />
              ${individual ? '' : '*'}
              <span class="help">
                <g:message code="request.step.message.${rqt.stepStates['iconito' + (individual ? '-' + params.type : '')].state}" />
              </span>
