...
<html>
  <head>
     <title>${message(code:'scjer.description')}</title>
  ...
  <body>
    ...
    <div id="request" class="main-box ${rqt.requestType.category.name}">
      <h2>
        ...
        <g:message code="scjer.label" /> <g:message code="message.number" args="[rqt.id]" />
      </h2>
      <p><g:message code="request.duration.label" /><strong> : <g:message code="scjer.duration.value" /></strong></p>
      <p>
      ...
      <div class="datas">
         <g:set var="requestTypeAcronym" value="scjer" scope="request" />
      ...
      <ul>
        ...
        <li class="${currentStep == 'sujet' ? 'current ' : ''}
          
            ${individual ? rqt.stepStates['sujet-' + params.type].state : rqt.stepStates['sujet'].state}
          
          ">
          <span class="number">1</span>
          <a
            <g:if test="${currentStep != 'sujet' && rqt.stepStates['sujet'].state != 'unavailable'}">
              href="${createLink(controller:'frontofficeRequest', action : 'edit', params:['id':rqt.id,'currentStep':'sujet'])}"
            </g:if>
          >
            
              <g:message code="${individual ? 'homeFolder.action.add' + org.apache.commons.lang.StringUtils.capitalize(params.type) : 'scjer.step.sujet.label'}" />
              ${individual ? '' : '*'}
              <span class="help">
                <g:message code="request.step.message.${rqt.stepStates['sujet' + (individual ? '-' + params.type : '')].state}" />
              </span>
        ...
        <li class="${currentStep == 'renseignements' ? 'current ' : ''}
          
            ${rqt.stepStates['renseignements'].state}
          
          ">
          <span class="number">2</span>
          <a
            <g:if test="${currentStep != 'renseignements' && rqt.stepStates['renseignements'].state != 'unavailable'}">
              href="${createLink(controller:'frontofficeRequest', action : 'edit', params:['id':rqt.id,'currentStep':'renseignements'])}"
            </g:if>
          >
            
              <g:message code="scjer.step.renseignements.label" /> *
              <span class="help">
                
                  <g:message code="request.step.message.${rqt.stepStates['renseignements'].state}" />
        ...
        <li class="${currentStep == 'reglements' ? 'current ' : ''}
          ...
          >
              ...
              <g:message code="scjer.step.reglements.label" /> *
              <span class="help">
