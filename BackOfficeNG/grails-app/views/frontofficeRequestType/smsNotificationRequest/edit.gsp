  ...
  <body>
    ...
    <div id="request" class="main-box">
      <h2>
        ...
        <g:message code="snr.label" /> <g:message code="message.number" args="[rqt.id]" />
      </h2>
      ...
      <div class="datas">
         <g:set var="requestTypeAcronym" value="snr" scope="request" />
      ...
      <ul>
        ...
        <li class="${currentStep == 'subscription' ? 'current ' : ''}
          
            ${individual ? rqt.stepStates['subscription-' + params.type].state : rqt.stepStates['subscription'].state}
          
          ">
          <span class="number">1</span>
          <a
            <g:if test="${currentStep != 'subscription' && rqt.stepStates['subscription'].state != 'unavailable'}">
              href="${createLink(controller:'frontofficeRequest', action : 'edit', params:['id':rqt.id,'currentStep':'subscription'])}"
            </g:if>
          >
            
              <g:message code="${individual ? 'homeFolder.action.add' + org.apache.commons.lang.StringUtils.capitalize(params.type) : 'snr.step.subscription.label'}" />
              ${individual ? '' : '*'}
              <span class="help">
                <g:message code="request.step.message.${rqt.stepStates['subscription' + (individual ? '-' + params.type : '')].state}" />
              </span>
