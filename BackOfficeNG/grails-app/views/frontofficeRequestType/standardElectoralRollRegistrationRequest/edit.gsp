...
<html>
  <head>
     <title>${message(code:'serrr.description')}</title>
  ...
  <body>
    ...
    <div id="request" class="main-box ${rqt.requestType.category.name}">
      <h2>
        ...
        <g:message code="serrr.label" /> <g:message code="message.number" args="[rqt.id]" />
      </h2>
      <p><g:message code="request.duration.label" /><strong> : <g:message code="serrr.duration.value" /></strong></p>
      <p>
      ...
      <div class="datas">
         <g:set var="requestTypeAcronym" value="serrr" scope="request" />
      ...
      <ul>
        ...
        <li class="${currentStep == 'inscription' ? 'current ' : ''}
          
            ${individual ? rqt.stepStates['inscription-' + params.type].state : rqt.stepStates['inscription'].state}
          
          ">
          <span class="number">1</span>
          <a
            <g:if test="${currentStep != 'inscription' && rqt.stepStates['inscription'].state != 'unavailable'}">
              href="${createLink(controller:'frontofficeRequest', action : 'edit', params:['id':rqt.id,'currentStep':'inscription'])}"
            </g:if>
          >
            
              <g:message code="${individual ? 'homeFolder.action.add' + org.apache.commons.lang.StringUtils.capitalize(params.type) : 'serrr.step.inscription.label'}" />
              ${individual ? '' : '*'}
              <span class="help">
                <g:message code="request.step.message.${rqt.stepStates['inscription' + (individual ? '-' + params.type : '')].state}" />
              </span>
        ...
        <li class="${currentStep == 'situation' ? 'current ' : ''}
          ...
          <a
            <g:if test="${currentStep != 'situation' && rqt.stepStates['situation'].state != 'unavailable'}">
              href="${createLink(controller:'frontofficeRequest', action : 'edit', params:['id':rqt.id,'currentStep':'situation'])}"
            </g:if>
          >
            
              <g:message code="serrr.step.situation.label" /> *
              <span class="help">
        ...
        <li class="${currentStep == 'radiation' ? 'current ' : ''}
          
            ${rqt.stepStates['radiation'].state}
          
          ">
          <span class="number">3</span>
          <a
            <g:if test="${currentStep != 'radiation' && rqt.stepStates['radiation'].state != 'unavailable'}">
              href="${createLink(controller:'frontofficeRequest', action : 'edit', params:['id':rqt.id,'currentStep':'radiation'])}"
            </g:if>
          >
            
              <g:message code="serrr.step.radiation.label" />
              <span class="help">
                
                  <g:message code="request.step.message.${rqt.stepStates['radiation'].state}" />
