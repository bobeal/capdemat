    ...
    <label for="distance" class="required"><g:message code="sgr.property.distance.label" /> *  <span><g:message code="sgr.property.distance.help" /></span></label>
            <select id="distance" name="distance" class="required  validate-not-first ${rqt.stepStates['calculationElements'].invalidFields.contains('distance') ? 'validation-failed' : ''}" title="<g:message code="sgr.property.distance.validationError" />">
