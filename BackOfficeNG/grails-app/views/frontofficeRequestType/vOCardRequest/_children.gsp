    ...
    <div class="yui-u">
      ...
      <select id="_individuals.children.${listIndex}.sex" name="_individuals.children[${listIndex}].sex" class="required validate-not-first ${stepStates != null && stepStates['children']?.invalidFields.contains('sex') ? 'validation-failed' : ''}" title="<g:message code="homeFolder.child.property.sex.validationError" />">
        ...
        <g:each in="${fr.cg95.cvq.business.users.SexType.allSexTypes}">
          <option value="fr.cg95.cvq.business.users.SexType_${it.toString()}" ${it == editList?.children?.sex ? 'selected="selected"': ''}>
              <g:capdematEnumToText var="${it.toString()}" i18nKeyPrefix="homeFolder.child.property.sex" />
          </option>
        </g:each>
      </select>
