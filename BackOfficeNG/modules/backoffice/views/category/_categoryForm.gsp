...
<form method="POST" id="categoryForm" action="<g:createLink action="save" />">
  ...
  <input type="text" name="name" class="required" size="40" 
      title="<g:message code='category.message.nameRequired'/>" value="${category?.name}"/>
  ...
  <input type="text" name="primaryEmail" size="40" class="required validate-email" 
      title="<g:message code="category.message.emailRequired" />" value="${category?.primaryEmail}"/>
  <div class="form-button">
    <input type="hidden" name="id" value="${category?.id}" />
    <g:if test="${editMode == 'create'}">
      <input type="button" id="submitSaveCategory" name="submitSaveCategory" 
          value="<g:message code="action.create" />" />
      <a id="cancelCreateCategory" href="<g:createLink action="list"/>">
