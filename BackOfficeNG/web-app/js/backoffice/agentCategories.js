(function() {
  ...
  zcb.agentCategory = function() {
    ...
    function updateCategoryAgentProfile(categoryId, action) {
      ...
      if (action != "unassociate") {
        var cloneNewProfileSpanEl = yud.getPreviousSibling(
            yus.query("input:checked", "categoryEditForm_" + categoryId, true)).cloneNode(true);
        currentLiEl.appendChild(cloneNewProfileSpanEl);
      }
    }

    function cancelCategoryEditForm(categoryId) {
      new yu.Element("category_" + categoryId).removeChild(yud
          .get("categoryEditForm_" + categoryId));
      // re-active add/edit item link button
      yud.removeClass(yus.query("a.currentEditItem", "category_" + categoryId, true),
          "currentEditItem");
    }

    function displayCategoryEditForm(e) {
      var categoryId = getCategoryId(e);
      var url = [zc['contextPath'],'/backoffice/category/editCategory/?id=',zcb.agentId,'&categoryId=',categoryId].join('');
      zct.doAjaxCall(url, null,
          function(o) {
            ...
            yud.addClass(
              yus.query("a." + yue.getTarget(e).className, "category_" + categoryId, true),
              "currentEditItem");
          },
          true);
    }
    ...
    return {
      ...
      init : function() {
        zcb.agentCategory.clickEvent = new zct.Event(zcb.agentCategory,
            zcb.agentCategory.getHandler);
        yue.on( [ 'sortCategoryForm', 'agentCategories' ], 'click',
            zcb.agentCategory.clickEvent.dispatch, zcb.agentCategory.clickEvent, true);
      },
      ...
      unassociate : function(e) {
        var categoryId = getCategoryId(e);
        var url = [zc['contextPath'],'/backoffice/category/unassociateAgent/?agentId=',zcb.agentId,'&categoryId=',categoryId].join('');
        zct.doAjaxCall(url, null, function(o) {
          ...
          if (response.status === "ok") {
            ...
            yud.replaceClass(yus.query("a.editItem", "category_" + categoryId, true), "editItem",
                "associate");
        ...
        zct.doAjaxFormSubmitCall("categoryEditForm_" + categoryId, null, function(o) {
          ...
          if (response.status === "ok") {
            ...
            if (firstAnchorEl.className === "associate") {
              ...
              new yu.Element(cloneAnchorEl).appendTo(yud.get("category_" + categoryId),
                  firstAnchorEl);
              // transform "asociate button" in "editItem
              // button"
      ...
      viewCategories : function(e) {
        ...
        var url = [zc['contextPath'],'/backoffice/category/categories/?id=',zcb.agentId,'&scope=',scope].join('');
        zct.doAjaxCall(url, [ scope ], displayCategories, true);
      }
    };
