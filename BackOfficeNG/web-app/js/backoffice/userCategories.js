(function(){
  ...
  zcb.userCategory = function() {
	...
	function updateCategoryUserProfile(categoryId, action) {
	...
	function displayCategoryEditForm(e) {
	    ...
	    zct.doAjaxCall("/editCategory/" + "?id=" + zcb.userId + "&categoryId=" + categoryId, null, function(o) {
	...
	return {
	  ...
	  init: function() {
	    zcb.userCategory.clickEvent = new zct.Event(zcb.userCategory, zcb.userCategory.getHandler);
	    yue.on('userCategories','click', zcb.userCategory.clickEvent.dispatch,
	        zcb.userCategory.clickEvent, true);
	  },
	  ...
	  unassociate : function(e) {
	    var categoryId = getCategoryId(e);
	    var url = "/unassociateCategory/?userId=" + zcb.userId + "&categoryId=" + categoryId;
	    zct.doAjaxCall(url, null, function(o) {
	        ...
	        if (response.status === "ok") {
	          ...
	          updateCategoryUserProfile(categoryId, "unassociate");
	        } else
	  ...
	  submitEditItem : function(e) {
	    ...
	    zct.doAjaxFormSubmitCall("categoryEditForm_" + categoryId, null, function(o) {
	        var response = ylj.parse(o.responseText);
	        if (response.status === "ok") {
	          updateCategoryUserProfile(categoryId, "editItem");
	  ...
	  viewCategories : function(scope) {
	    zct.doAjaxCall(
	        "/categories/?id=" + zcb.userId + "&scope=" + scope,
  ...
  YAHOO.util.Event.onDOMReady(zcb.userCategory.init);
