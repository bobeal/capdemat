...
(function() {
  ...
  zcfr.SaintouenInscriptionScolaireGlobaleRequest = function() {
    ...
    var toogleError = function(select) {
      if (!yud.hasClass(select, 'unactive')
        ...
        yud.get('stepForm-error').innerHTML = zcfr.SaintouenInscriptionScolaireGlobaleRequest.errorMessage;
    ...
    return {
      init: function() {
        if (!yud.get('subjectId'))
          return;
        updateLayout();
        zcfr.SaintouenInscriptionScolaireGlobaleRequest.initSchools();
        yue.on('subjectId','change', zcfr.SaintouenInscriptionScolaireGlobaleRequest.initSchools);
      },

      errorMessage : 'Aucune école de secteur ne vous est proposée. Contactez votre Mairie.',
      ...
      initSchools: function() {
        ...
        if (childId !== '') {
          zct.doAjaxCall(zenexity.capdemat.contextPath + '/frontoffice/saintouenInscriptionScolaireGlobale/schoolSectors/?childId=' + childId, null, function(o){
            schoolSector.initOptions(o.responseText);
