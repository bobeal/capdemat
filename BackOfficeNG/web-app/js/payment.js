...
function doSearchPayment(filter, sort) {
	var queryUrl = YAHOO.capdematBo.baseUrl + "/loadPayments?" +  collectSearchFormValues('paymentForm');	
	if (filter) {
		var el = document.getElementById(filter);
		queryUrl += '&' + filter + '=' + el.value;
	}
    doAjaxFormSubmitCall(handleSearchPaymentSuccess, ['search-results'], 'paymentForm');	
}
