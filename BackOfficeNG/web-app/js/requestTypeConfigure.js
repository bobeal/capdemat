function initRequestTypeConfigure() {
	...
	function selectRequestTypeSubmenu(type, args) {
    	switchSelectedItemDisplay(args[0], 'selected');
    }
    ...
    function loadRequestTypeSubmenu(type, args) {
    	...
    	if (args[1] === 'general') {
    	    ...
    	    doAjaxCall(url, handleLoadRequestTypeSubmenuFormSuccess, [args[2], args[1], 'requestTypeAlerts']);
	        url = '/loadFormsArea/' + args[2];
    	    doAjaxCall(url, handleLoadRequestTypeSubmenuFormSuccess, [args[2], args[1], 'requestTypeForms']);  			
    	} else if (args[1] === 'seasons') {
	        url = '/loadSeasonsArea/' + args[2];
    	    doAjaxCall(url, handleLoadRequestTypeSubmenuFormSuccess, [args[2], args[1], 'requestTypeSeasons']);
    	}
    }
