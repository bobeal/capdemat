function initRequestTypeSeasons(requestTypeId) {
	...
	function onSubmitSaveSeasonClick(ev){
		doAjaxFormSubmitCall(handleSaveRequestTypeSeasonsSuccess,null,'requestTypeSeasonsForm');
	}
	var handleSaveRequestTypeSeasonsSuccess = function(o) {
		
		var response = YAHOO.lang.JSON.parse(o.responseText);
		if (response.status == "ok") {
		     doAjaxCall('/loadRequestTypeSeasonsList/' +  requestTypeId , 
		     handleLoadRequestTypeSeasonsListSuccess,[response.seasonUuid,response.seasonLabel]);
        }else {
...
function askSeasonDeleteConfirmation(requestTypeId, seasonUuid, seasonLabel) {  
    ...
    var handleConfirmDelete = function() {
        doAjaxCall('/deleteSeasons/' + requestTypeId + '/' + seasonUuid, handleDeleteSeasonSuccess);
        this.hide();
    }
    ...
    if (!deleteConfirmationDialog) {
        ...
        deleteConfirmationDialog = 
            new YAHOO.capdematBo.deleteConfirmationDialog(null,handleConfirmDelete,body);
    }
