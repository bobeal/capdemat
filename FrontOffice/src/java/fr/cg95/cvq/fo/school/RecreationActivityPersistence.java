...
import fr.cg95.cvq.fo.school.recreation.form.Authorized;
import fr.cg95.cvq.fo.school.recreation.form.Contact;
...
public class RecreationActivityPersistence extends IPersistence {
    ...
    public RequestType createRequest(HttpServletRequest request, Request cvqRequest) {
        ...
        if (!cvqRequest.isInitialised()) {
            ...
            Collection activityList = BusinessManager.getReferentialList("RecreationActivity");
        ...
        if (request.getParameter(ProcessWizardState.INDEX_REQUEST_PARAMETER) == null) {
            request.setAttribute(ProcessWizardState.NAME_REQUEST_PARAMETER, "eRecreation");
            ...
            RecreationActivityRegistrationRequest rarr = RecreationActivityRegistrationRequest.Factory.newInstance();
    
            rarr.addNewOtherIndividual();
            rarr.getOtherIndividualArray(0).setHomePhone("");
            rarr.getOtherIndividualArray(0).setOfficePhone("");
            
            initializeSubject(rarr, child.getId());
            ...
            if ((srr != null) && (srr.getUrgencyPhone() != null))
                rarr.setUrgencyPhone(srr.getUrgencyPhone());
            else
                rarr.setUrgencyPhone("");
            ...
            rarr.setRecreationActivityArray(activities);
    ...
    public Long validateRequest(HttpServletRequest request, Request cvqRequest, Object xmlRequestData)
        ...
        RecreationActivityRegistrationRequestDocument.RecreationActivityRegistrationRequest xmlRequest = (RecreationActivityRegistrationRequestDocument.RecreationActivityRegistrationRequest) xmlRequestData;

        xmlRequest.setOtherIndividualArray(saveContactsAndAuthorized(cvqRequest.getList("contact"),
                cvqRequest.getList("authorized")));
        
        RecreationActivityRegistrationRequestDocument document = RecreationActivityRegistrationRequestDocument.Factory
                .newInstance();
        document.setRecreationActivityRegistrationRequest(xmlRequest);

        IRecreationActivityRegistrationRequestService service = (IRecreationActivityRegistrationRequestService) BusinessManager
                .getAc().getBean(IRecreationActivityRegistrationRequestService.SERVICE_NAME);
