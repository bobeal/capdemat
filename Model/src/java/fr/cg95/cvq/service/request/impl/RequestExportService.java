...
public class RequestExportService implements IRequestExportService {
    ...
    @Override
    public XmlObject fillRequestXml(Request request) throws CvqException {
        XmlObject result = request.modelToXml();
        RequestType xmlRequestType = null;
        try {
            xmlRequestType = (RequestType) result.getClass().getMethod("get" + result.getClass()
                    .getSimpleName().replace("DocumentImpl", "")).invoke(result);
