 ...
 */
public class DeathDetailsRequestServiceTest extends ServiceTestCase {

    protected IDeathDetailsRequestService iDeathDetailsRequestService;

    protected void onSetUp() throws Exception {
    	super.onSetUp();
        ConfigurableApplicationContext cac = getContext(getConfigLocations());
        iDeathDetailsRequestService = 
            (IDeathDetailsRequestService) cac.getBean(StringUtils.uncapitalize("DeathDetailsRequest") + "Service");
    }
    ...
    protected void completeValidateAndDelete(DeathDetailsRequest request) 
        ...
        iDeathDetailsRequestService.addDocument(request.getId(), documentId);
        Set documentsSet =
            iDeathDetailsRequestService.getAssociatedDocuments(request.getId());
        ...
        iDeathDetailsRequestService.complete(request.getId());
        iDeathDetailsRequestService.validate(request.getId());
        ...
        // delete request
        iDeathDetailsRequestService.delete(request.getId());
    }
	...
	request.setRequester(homeFolder.getHomeFolderResponsible());
        DeathDetailsRequestFeeder.setSubject(request, homeFolder);

        Set authorizedSubjects = iDeathDetailsRequestService.getAuthorizedSubjects(homeFolderId).keySet();

	DeathDetailsRequestDocument requestDoc =
		(DeathDetailsRequestDocument) request.modelToXml();
	Long requestId = iDeathDetailsRequestService.create(requestDoc.getDomNode());

        // close current session and re-open a new one
        continueWithNewTransaction();
        
        Map newAuthorizedSubjectsMap = iDeathDetailsRequestService.getAuthorizedSubjects(homeFolderId);
	...
	if (!iDeathDetailsRequestService.supportUnregisteredCreation())
	    return;
	...
	startTransaction();
        ...
        DeathDetailsRequestFeeder.setSubject(request, null);

        DeathDetailsRequestDocument requestDoc = 
            (DeathDetailsRequestDocument) request.modelToXml();
        Long requestId =
             iDeathDetailsRequestService.create(requestDoc.getDomNode());
