...
public class PersonalDetailsRequestServiceTest extends ServiceTestCase {
    ...
    protected void onSetUp() throws Exception {
        ...
        iPersonalDetailsRequestService = 
            (IPersonalDetailsRequestService) cac.getBean(StringUtils.uncapitalize("PersonalDetailsRequest") + "Service");
    }
	...
	request.setRequester(homeFolder.getHomeFolderResponsible());
        PersonalDetailsRequestFeeder.setSubject(request, homeFolder);

        Set authorizedSubjects = iPersonalDetailsRequestService.getAuthorizedSubjects(homeFolderId).keySet();

	PersonalDetailsRequestDocument requestDoc =
		(PersonalDetailsRequestDocument) request.modelToXml();
	Long requestId = iPersonalDetailsRequestService.create(requestDoc.getDomNode());

        // close current session and re-open a new one
        continueWithNewTransaction();
        
        Map newAuthorizedSubjectsMap = iPersonalDetailsRequestService.getAuthorizedSubjects(homeFolderId);
	...
	startTransaction();
        ...
        PersonalDetailsRequestFeeder.setSubject(request, null);

        PersonalDetailsRequestDocument requestDoc = 
            (PersonalDetailsRequestDocument) request.modelToXml();
        Long requestId =
             iPersonalDetailsRequestService.create(requestDoc.getDomNode());
