 ...
 */
public class PlaceReservationRequestServiceTest extends ServiceTestCase {

    protected IPlaceReservationRequestService iPlaceReservationRequestService;

    protected void onSetUp() throws Exception {
    	super.onSetUp();
        ConfigurableApplicationContext cac = getContext(getConfigLocations());
        iPlaceReservationRequestService = 
            (IPlaceReservationRequestService) cac.getBean(StringUtils.uncapitalize("PlaceReservationRequest") + "Service");
    }

    protected PlaceReservationRequest fillMeARequest() throws CvqException {
    ...
    protected void completeValidateAndDelete(PlaceReservationRequest request) 
        ...
        iPlaceReservationRequestService.addDocument(request.getId(), documentId);
        Set documentsSet =
            iPlaceReservationRequestService.getAssociatedDocuments(request.getId());
        ...
        // delete request
        iPlaceReservationRequestService.delete(request.getId());
    }
	...
	request.setRequester(homeFolder.getHomeFolderResponsible());
        PlaceReservationRequestFeeder.setSubject(request, homeFolder);

        Set authorizedSubjects = iPlaceReservationRequestService.getAuthorizedSubjects(homeFolderId).keySet();
	...
	Long requestId = iPlaceReservationRequestService.create(requestDoc.getDomNode());
        ...
        Map newAuthorizedSubjectsMap = iPlaceReservationRequestService.getAuthorizedSubjects(homeFolderId);
	...
	if (!iPlaceReservationRequestService.supportUnregisteredCreation())
	    return;
	...
	startTransaction();
        ...
        PlaceReservationRequestFeeder.setSubject(request, null);

        PlaceReservationRequestDocument requestDoc = 
            (PlaceReservationRequestDocument) request.modelToXml();
        Long requestId =
             iPlaceReservationRequestService.create(requestDoc.getDomNode());
